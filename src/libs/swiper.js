...
!(function (e, t) {
	...
	function $({ swiper: e, runCallbacks: t, direction: s, step: a }) {
		...
		let n = s;
		if (
			(n || (n = i > r ? 'next' : i < r ? 'prev' : 'reset'), e.emit(`transition${a}`), t && i !== r)
		) {
		...
		function ({ swiper: e, extendParams: t, on: s }) {
			...
			function v() {
				...
				e.slides.each((s, a) => {
					...
					c(
						i,
						t.slideLabelMessage.replace(/\{\{index\}\}/, r + 1).replace(/\{\{slidesLength\}\}/, u)
					);
				}),
					...
					setTranslate: () => {
						...
						for (let l = 0; l < t.length; l += 1) {
							...
							let f = 1,
								...
								v = 8 - 0.75 * Math.abs(c);
							const w =
									(l === s || l === s - 1) && c > 0 && c < 1 && (r || e.params.cssMode) && n < i,
